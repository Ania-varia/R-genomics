...
In R console, the ">" sign is a prompt - a signal that R is ready to receive input from the user. R automatically starts a line with ">". To make R to react to your input, just press `ENTER`.
...
- What happens if we assign a different value to `a` and then re-add `a` and `b`?
...
You can append new elements to your vector by using the `c()` function:
